...
public class RequestClaimController {
    ...
    @PreAuthorize("hasRole('CUSTOMER')")
    public RequestClaim ajouterRequestClaimEtAffecterOrder(@AuthenticationPrincipal UserDetails userDetails,@RequestBody RequestClaim claim,@PathVariable("idOrder") int id)
    {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String currentUserName = authentication.getName();
        User currentUser = userRepo.findUserByUsername(currentUserName);
 ...
 /*   @PutMapping("/addNAffectUSER/{idUser}")
    ...
    public RequestClaim updateRequestClaim(@AuthenticationPrincipal UserDetails userDetails,@RequestBody RequestClaim claim, @PathVariable("idRequestClaim") int idRequestClaim){
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String currentUserName = authentication.getName();
        User currentUser = userRepo.findUserByUsername(currentUserName);
        if (currentUser==null) {
        ...
        return RequestClaimService.updateRequestClaim(userDetails,claim, idRequestClaim);
    }
    ...
    void removeClaim(@AuthenticationPrincipal UserDetails userDetails, @PathVariable("idRequestClaim") Integer idRequestClaim) {
        RequestClaimService.removeClaim(userDetails, idRequestClaim);
    }
    ...
    public ResponseEntity<String> traiterReclamation(@AuthenticationPrincipal UserDetails userDetails,@RequestBody RequestClaim reclamation) {
        return RequestClaimService.traiterReclamation(userDetails,reclamation);
