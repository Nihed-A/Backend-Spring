...
//        return requestClaimRepository.save(claim);
    ...
    @Override
    public void removeClaim(@AuthenticationPrincipal UserDetails userDetails, Integer idRequestClaim) {
   ...
   /* @Override
    ...
    @Override
    public RequestClaim ajouterRequestClaimEtAffecterOrder(@AuthenticationPrincipal UserDetails userDetails,RequestClaim claim,int id) {
        String currentUser = userDetails.getUsername();
        User user1 = userRepo.findUserByUsername(currentUser);
        claim.setUser(user1);
    ...
    public ResponseEntity<String> traiterReclamation(@AuthenticationPrincipal UserDetails userDetails, RequestClaim reclamation) {
        String currentUser = userDetails.getUsername();
        User user1 = userRepo.findUserByUsername(currentUser);
        reclamation.setUser(user1);
        ...
        try {
            ...
            if (reclamation.getTypeClaim().equals(TypeClaim.Product_Canceling)) {
                ...
                // ...
                reclamation.setStatus(Status.in_progress);
            }  else if (reclamation.getTypeClaim().equals(TypeClaim.Incomplete_Package)) {
                    // Traiter la réclamation de Wrong_Product_Reference
                    // ...
                    reclamation.setStatus(Status.in_progress);
        ...
        for (RequestClaim requestClaim : requestClaims) {
            ...
            if (hasBadWords) {
                if (requestClaim.getUser().getNumberOfAlerts() <= maxAlerts ) {
                    ...
                    requestClaim.getUser().setVerified(false);
                }
    ...
    public void UpdateSystemReclamation() {
        ...
        for (RequestClaim reclamation : reclamations) {
            ...
            Message message = Message.creator(new com.twilio.type.PhoneNumber("+216" + reclamation.getUser().getNumTel()),
                            new com.twilio.type.PhoneNumber("52110900"),
